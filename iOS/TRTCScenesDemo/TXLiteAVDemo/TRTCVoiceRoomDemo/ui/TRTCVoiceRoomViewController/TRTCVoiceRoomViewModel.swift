...
extension TRTCVoiceRoomViewModel {
    ...
    private func audienceClickItem(model: SeatInfoModel) {
        ...
        if model.isUsed {
            if dependencyContainer.userId == model.seatUser?.userId ?? "" {
                // 点击自己的头像，弹出下麦对话框
                viewResponder?.showActionSheet(actionTitles: ["leave"], actions: { [weak self] (index) in
                    guard let `self` = self else { return }
                    self.leaveSeat()
                })
            } else {
            ...
            if mSelfSeatIndex != -1 {
                viewResponder?.showToast(message: "You are already in the seat No. \(mSelfSeatIndex)")
                return
            }
    ...
    private func anchorClickItem(model: SeatInfoModel) {
        if model.isUsed {
            ...
            viewResponder?.showActionSheet(actionTitles: ["\(isMute ? "lift" : "Mute") him/her", "Ask him/her to leave"], actions: { [weak self] (index) in
                guard let `self` = self else { return }
                if index == 0 {
                    // 禁言
        ...
        if !model.isClosed {
            // 没人的话弹出封禁麦位和请人上麦
            viewResponder?.showActionSheet(actionTitles: ["Invite him/her", "locked the seat"], actions: { [weak self] (index) in
                guard let `self` = self else { return }
                if index == 0 {
...
extension TRTCVoiceRoomViewModel: TRTCVoiceRoomDelegate {
    ...
    func onAnchorEnterSeat(index: Int, user: VoiceRoomUserInfo) {
        ...
        showNotifyMsg(messsage: "\(user.userName) is on the no.\(index) seat")
    ...
    func onAudienceEnter(userInfo: VoiceRoomUserInfo) {
        showNotifyMsg(messsage: "\(userInfo.userName) enter room.")
        // 主播端(房主)
