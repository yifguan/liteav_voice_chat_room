...
extension TRTCVoiceRoomRootView: TRTCVoiceRoomViewResponder {
    ...
    func onSeatMute(isMute: Bool) {
        if isMute {
            makeToast("Muted by the host.", duration: 0.3)
        } else {
            makeToast("Lifted by the host.", duration: 0.3)
    ...
    func showAlert(info: (title: String, message: String), sureAction: @escaping () -> Void, cancelAction: (() -> Void)?) {
        ...
        let sureAlertAction = UIAlertAction.init(title: "Accept", style: .default) { (action) in
            sureAction()
        ...
        let cancelAlertAction = UIAlertAction.init(title: "Reject", style: .cancel) { (action) in
            cancelAction?()
            alertController.dismiss(animated: false) { [weak self] in
                guard let `self` = self else { return }
                self.alertController = nil
    ...
    func showActionSheet(actionTitles: [String], actions: @escaping (Int) -> Void) {
        let actionSheet = UIAlertController.init(title: " Please select", message: "", preferredStyle: .actionSheet)
